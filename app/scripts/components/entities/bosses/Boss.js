Crafty.c('Boss', {
    init: function () {
        this.requires('2D, WebGL, Collision, Tween, Choreography, ViewportFixed, Hideable, Flipable, Scalable, Hostile');
        this.attr({
            pointsOnHit: 5,
    ...
    boss: function (options) {
        ...
        this.healthBar = Crafty.e('BossBar').render(this.name, {
            current: this.health,
            max: this.maxHealth
        });

        Crafty.trigger('BossSpawned', this);

        this.onHit(options.projectile, function(e) {
            var bullet;
            if (Game.paused) {
                return;
            }
        ...
        this.onHit('Explosion', function (e) {
    ...
    absorbDamage: function (cause) {
        ...
        if (this.health <= 0) {
            Crafty.trigger('BossDestroyed', this);
